...
- 为计算 Token 消耗及独立的速度挡位估算，会顺序扫描本地 JSONL，但仅提取并在内存中保留用于统计的 `token_count`、模型标识、`service_tier`、会话边界和时间戳元数据；
...
窗口设置仅保存在 `%LOCALAPPDATA%\CodexQuotaWidget\settings.json`；删除该文件即可重置本工具的本地设置。原有金额为依据公开 Standard API 单价计算的本地等价估算；速度挡位金额则依据日志中的线程 `service_tier` 设置单独估算，并对缺失字段或无公开价格的内部模型采用 README 所述参考价兜底。两者都不是订阅账单，统计结果不会上传。
